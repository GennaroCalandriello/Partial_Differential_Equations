...
def animate_matplotlib(x, y, u_della_vita):
    fig = plt.figure()
...
def static_plot(x, y, u, t):
    fig = plt.figure(figsize=(11, 8), dpi=100)
    ax = fig.gca(projection='3d')
    X, Y = np.meshgrid(x, y)
    ax.plot_surface(X, Y, u[:, :], cmap=cm.spring, rstride=1, cstride=1)
    # ax.set_zlim(-1, 1)
    ax.set_title(f'Time step: {t}', fontsize=15)
    ax.set_xlabel('x', fontsize=12)
    ax.set_ylabel('y', fontsize=12)
    ax.set_zlabel('u(x, y, t)', fontsize=12)
    fig.savefig(f'OneDrive/Desktop/Github_projects/Van_der_Pol_Fokker_Planck/VdP_explicit/plots/time_cos/time_cos_{t}') #change the path
    # plt.show()
